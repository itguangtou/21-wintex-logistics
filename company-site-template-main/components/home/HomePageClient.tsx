...
export default function HomePageClient({ locale, newsItems, contact }: HomePageClientProps) {
  ...
  const [newsInViews, setNewsInViews] = useState([false, false, false, false]);
  ...
  // news images
  useEffect(() => {
    setNewsInViews(Array(homeNews.length).fill(false));
              ...
              >
                  ...
                  <div className="w-full rounded-lg mb-3 bg-brand-accent-50 p-[11.7px] flex items-center justify-center cursor-pointer aspect-[4/3] overflow-hidden">
                    <img
                      ...
                      style={{
                        ...
                        transition: newsInViews[i]
                          ? `opacity 0.8s ease-out ${i * 0.1}s, transform 0.8s ease-out ${i * 0.1}s`
                          : 'none',
                      }}
                    />
                  </div>
                ...
                <div
                  ...
                  style={{
                    ...
                    transform: newsInViews[i] ? 'translateY(0)' : 'translateY(30px)',
                    transition: newsInViews[i]
                      ? `opacity 0.8s ease-out ${i * 0.1 + 0.2}s, transform 0.8s ease-out ${i * 0.1 + 0.2}s`
                      : 'none',
                  }}
                >
                  ...
                  <p className="text-gray-400 line-clamp-2 mb-1">{news.preview}...</p>
                  {news.date && (
                    <p className="text-gray-400 text-right text-sm mt-2">{news.date}</p>
                  )}
                </div>
              </div>
            ))}
