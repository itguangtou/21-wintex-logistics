...
export type EquipmentGalleryItem = {
  image: string;
  name: LocaleText;
};

export type EquipmentDetailCard = {
  title: LocaleText;
  desc: LocaleText;
};
...
export type EquipmentPageContent = {
  ...
  gallery: EquipmentGalleryItem[];
  detailModule1: {
    row1: EquipmentDetailCard[];
    row2: EquipmentDetailCard[];
  };
  detailModule2: {
    row1: EquipmentDetailCard[];
    row2: EquipmentDetailCard[];
...
/** 与现网硬编码装备清单页一致；固定条数，管理端只编辑不增删 */
export const DEFAULT_EQUIPMENT_CONTENT: EquipmentPageContent = {
  pageTitle: {
    zh: '装备清单',
    en: 'Equipment List',
  },
